...
if __name__ == '__main__':
    ...
    cformatV(sys.stdout, 'conv_rnnrf_flipflop_{}b'.format(modelid), network.sublayers[0].conv.bias.reshape(-1))
    ...
    print_gru(gru1, 'gruB1_rnnrf_flipflop_{}'.format(modelid))

    """  Forward GRU (second layer)
    """
    gru2 = network.sublayers[2]
    print_gru(gru2, 'gruF2_rnnrf_flipflop_{}'.format(modelid))

    """ backward GRU(third layer)
    """
    gru3 = network.sublayers[3].layer
    print_gru(gru3, 'gruB3_rnnrf_flipflop_{}'.format(modelid))

    """  Forward GRU (fourth layer)
    """
    gru4 = network.sublayers[4]
    print_gru(gru4, 'gruF4_rnnrf_flipflop_{}'.format(modelid))

    """ backward GRU(fifth layer)
    """
    gru5 = network.sublayers[5].layer
    print_gru(gru5, 'gruB5_rnnrf_flipflop_{}'.format(modelid))
    """ Global norm layer
    """
    gnlayer = network.sublayers[6]
    nstate = gnlayer.linear.weight.shape[0]
    cformatM(sys.stdout, 'FF_rnnrf_flipflop_{}W'.format(modelid), gnlayer.linear.weight)
    cformatV(sys.stdout, 'FF_rnnrf_flipflop_{}b'.format(modelid), gnlayer.linear.bias)
